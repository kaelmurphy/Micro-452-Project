    ...
    def __init__(self, io, node_id=1, refreshing_rate=None):
        ...
        def do_refresh():
            while not self.terminating:
                ...
                self.get_variables()
        self.refresh_thread = threading.Thread(target=do_refresh, daemon=True)
...
if __name__ == "__main__":
    import time
    with Thymio.serial(port='COM6', refreshing_rate=0.1) as thymio:
        i = 4
        while i:
            try:
                print(thymio["prox.horizontal"])
            except KeyError:
                pass
            time.sleep(0.2)
            i -= 1
